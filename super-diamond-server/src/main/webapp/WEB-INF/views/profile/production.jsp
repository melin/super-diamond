...
<table class="table table-striped table-bordered">
    ...
    <c:forEach items="${configs}" var="config">
        <tr id='row-<c:out value="${config.CONFIG_ID}"/>'   <c:if
                test="${fn:startsWith(config.REAL_PRODUCTION_VALUE, 'RPF:')}"> style=" background: yellow; color: red; font-weight: bold;" title="配置替换失败" </c:if>>
            ...
            <td title='<c:out value="${fn:replace(config.REAL_PRODUCTION_VALUE,\"RPF:\",\"\")}"/>'>
                <script type="text/javascript">
