...
<table class="table table-striped table-bordered">
    ...
    <c:forEach items="${configs}" var="config">
        <tr id='row-<c:out value="${config.CONFIG_ID}"/>'   <c:if
                test="${fn:startsWith(config.REAL_TEST_VALUE, 'RPF:')}"> style=" background: yellow; color: red; font-weight: bold;" title="配置替换失败" </c:if> >
            ...
            <td title='<c:out value="${fn:replace(config.REAL_TEST_VALUE,\"PRF:\",\"\")}"/>'>
                <script type="text/javascript">
                    var value = '<c:out value="${config.REAL_TEST_VALUE}"/>';
